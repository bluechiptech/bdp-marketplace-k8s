...
{{- if and .Values.sidecar.dashboards.enabled (not .Values.sidecar.dashboards.initDashboards) }}
  - name: {{ include "grafana.name" . }}-sc-dashboard
    {{- /* Task G1: air-gapped, digest-pinned — sourced from global.azure.images,
           keyed by THIS container's rendered name (grafana-sc-dashboard) */}}
...
{{- if and .Values.sidecar.datasources.enabled (not .Values.sidecar.datasources.initDatasources) }}
  - name: {{ include "grafana.name" . }}-sc-datasources
    {{- /* Task G1: air-gapped, digest-pinned — sourced from global.azure.images,
           keyed by THIS container's rendered name (grafana-sc-datasources) */}}
    image: "{{ (index .Values.global.azure.images "grafana-sc-datasources").registry }}/{{ (index .Values.global.azure.images "grafana-sc-datasources").image }}@{{ (index .Values.global.azure.images "grafana-sc-datasources").digest }}"
    imagePullPolicy: {{ .Values.sidecar.imagePullPolicy }}
    env:
      {{- range $key, $value := .Values.sidecar.datasources.env }}
      - name: "{{ $key }}"
